...
circRNA_expression_raw <- read.table(paste0(output_dir, "/results/circRNA/circRNA_counts_all_samples_raw.tsv"),header = T, stringsAsFactors = F)
...
#correlations <- read.table(paste0(output_dir, "/results/sponging/filtered_circRNA_miRNA_correlation.tsv"), sep = "\t", stringsAsFactors = F)
...
png(paste0(plot_folder, "/top8_neg_0.05_0.1_50.png"), width = 1200, height = 700)
...
ggsave(paste0(plot_folder, "/highest_pos_corr_",pval_filter,"_", ratio_filter, "_", bind_sites_filter, ".png"), width = 8, height = 4)
